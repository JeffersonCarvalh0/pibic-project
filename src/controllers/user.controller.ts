...
export class UserController {
    public static login(req: Request, res: Response) {
        passport.authenticate('local', { session: false }, (err: Error, user: IUser, info) => {
            ...
            res.json({ data: {"token": token}, message: info.message, errors: err});
        })(req, res);
    }
    ...
    public static async register(req: Request, res: Response) {
        try {
            ...
            user.save((err: Error, doc: IUser) => {
                ...
                if (doc) doc.password = "";
                let message = err ? err.message : "user created";
                res.json({data: doc, message: message, errors: err});
            });
        } catch (err) { throw err; }
    }
    ...
    public static async remove(req: Request, res: Response) {
        passport.authenticate('jwt', {session: false}, async(err: Error, user: IUser, info) => {
            ...
            else res.json({ data: null, message: info.message, errors: err });
        })(req, res);
    }
}
